...
**核心作用**：**提升页面滚动（Scroll）的流畅度**。

**原理**：默认情况下，浏览器不知道你的事件处理函数是否会调用 `preventDefault()` 来阻止滚动。因此，它必须等待 JS 函数执行完毕才开始滚动，如果js执行的时候有大量操作会影响性能。通过设置 `{ passive: true }`，你相当于明确告诉浏览器：“我**不会**阻止滚动”。

**效果**：浏览器可以立即开始滚动，无需等待 JS 执行，解决了移动端常见的滑动延迟（Jank）问题
