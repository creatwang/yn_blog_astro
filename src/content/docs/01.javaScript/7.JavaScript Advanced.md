...
- 读写分离：读取的时候统一读取，修改样式时尽量一次性修改

浏览器会等待当前的 JavaScript 代码执行完毕，或者等到一个预定的时间片（通常是 **16ms**，即一帧的时间），然后一次性把队列里所有的修改拿出来，**只做一次重排**。

- 如果这类操作出现在**循环**里，就会演变成**布局抖动（Layout Thrashing）**：

- 宏观上时异步，微观上时同步： 当你修改了 DOM 样式，浏览器不会立刻开启显卡去画图，而是把任务排进**渲染队列**。

- 但是你如果刚刚写完就立刻读，那么浏览器需要保证你拿到的值是正确的所以，要立刻重排一下

~~~javascript
// ❌ 错误示范：读 -> 写 -> 读 -> 写
items.forEach((el) => {
  const width = el.offsetWidth;         // 【读】触发布局计算
  el.style.width = (width + 10) + 'px'; // 【写】标记布局失效
});
// 循环 100 次，浏览器就可能重排 100 次


// ✅ 正确示范：先统一读，再统一写
const widths = items.map(el => el.offsetWidth); // 【集中读】浏览器只计算一次布局

items.forEach((el, i) => {
  el.style.width = (widths[i] + 10) + 'px';     // 【集中写】在 JS 执行完后统一渲染
});
~~~
...
3. `opacity` 动画转换时；
4. `position`: `fixed`
5. `will-change`：提前告诉浏览器：这个元素**马上**要变了 “别等我动的时候再建胶片，你**现在**就先给我在 GPU 里占个位。”
...
#### 4.2、will-change  性能优化

`will-change` 的核心使用场景可以概括为：针对已知的高频、复杂动画进行提前性能扩容。

由于它会消耗显存（GPU 资源），所以它不应该用于“常态样式”，而应该用于以下特定场景：

1. 复杂动画的“防抖”

当你点击某个元素（如你的 **SKU 规格块**），需要执行缩放、位移等动画时，如果该元素内部 DOM 结构复杂，浏览器在动画开始瞬间可能会卡顿。

- **做法**：在鼠标悬浮（hover）或触摸开始（touchstart）时动态添加 `will-change`。
- **场景**：弹出层（Modal）、侧边栏抽屉（Drawer）、规格选中时的缩放效果。
- 超长列表的滚动优化

在移动端处理上千条数据的长列表时，滚动可能会出现白屏或掉帧。

- **场景**：`will-change: scroll-position;`
- **作用**：告诉浏览器提前准备好滚动容器的内容渲染，让滑动更“丝滑”。
- 固定定位元素的闪烁优化

某些 `fixed` 布局的头部或底部导航，在页面滚动时可能会产生抖动或闪烁。

- **场景**：`will-change: transform;` 或 `top;`
- **作用**：将固定元素提升到独立渲染层，避免受主体内容重排的影响。
- 配合 GPU 密集型操作

如果你的页面使用了滤镜（filters）或复杂的透明度变化。

- **场景**：`will-change: opacity, filter;`
- **作用**：避免 CPU 实时计算复杂的像素合成。
...
> 由于try catch是同步执行的代码，所以无法直接捕获异步异常，可以使用await

- **原理**：`await` 会暂停当前函数的执行，直到异步任务有了结果。
- **本质**：JavaScript 引擎会将 `await` 后面的代码包装成一个微任务（Microtask）。如果异步任务失败，引擎会**手动**把这个错误“抛回”到当前的 `try-catch` 上下文中。

~~~ts
try {
  await someAsyncFunc(); // 这里的 await 相当于“在原地等结果”
} catch (e) {
  console.log("抓到了！");
}

try {
  Promise.reject("错误"); // 这是一个异步对象
} catch (e) {
  // 依然抓不到！
}
...
try {
  oldSDK.scan((result) => {
    throw new Error("回调里报错"); // 这里的报错会直接导致程序崩溃，catch 抓不到
  });
} catch (e) {}
~~~
...
### 例子 1：函数可能同步抛错，也可能异步失败

~~~ts
function maybeTask(mode) {
  if (mode === 'sync-throw') throw new Error('sync error');
  if (mode === 'async-reject') return Promise.reject(new Error('async error'));
  return 'ok';
}

// 用 Promise.try（es2025新增） 统一处理
Promise.try(() => maybeTask('sync-throw'))
  .then((res) => console.log('success:', res))
  .catch((err) => console.log('caught:', err.message));

// 当然这样也可以

async function maybeTask(mode) {
try {
  if (mode === 'sync-throw') throw new Error('sync error');
  if (mode === 'async-reject') return await Promise.reject(new Error('async error'));
} catch (e) {
}
return 'ok';
}

// 输出: caught: sync error

// 注意：抛出的错误要在promise链中这样的话要在包裹一层promise reject
Promise.try(() => {
  setTimeout(() => {
    throw new Error('timer error'); // 不在当前 Promise 链里
  }, 0);
}).catch(() => {});
~~~
...
# 第十三章、debounce，throttle，deepCopy，event-bus
...
temp.innerHTML = '<li>Item 1</li><li>Item 2</li>';
const fragment = temp.content; // 得到一个 DocumentFragment
...
//2. script 标签 脚本一旦下载完成，浏览器会立即暂停当前的 HTML 解析（或主线程任务），先执行这个 JS
//3. new Function
